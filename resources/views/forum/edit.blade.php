<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Your Discussion</h2>
    </x-slot>

    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('forum-posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <select name="category" required 
                        ...
                        <option value="Spreading the Word" {{ $post->category === 'Spreading the Word' ? 'selected' : '' }}>Spreading the Word</option>
                        <option value="Zero Waste" {{ $post->category === 'Zero Waste' ? 'selected' : '' }}>Zero Waste</option>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="content" rows="5" placeholder="Write your post content here..." 
                              class="w-full border p-2 rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>{{ old('content', $post->content) }}</textarea>
                </div>
                ...
                <div class="mb-4">
                    ...
                    @if ($post->image_path)
                        <div class="mt-4">
                            <img src="{{ asset('storage/' . $post->image_path) }}" alt="Post Image" class="rounded w-48 h-48 object-cover">
                            <p class="text-sm text-gray-500 mt-2">Current image: {{ $post->image_path }}</p>
                        </div>
                    @endif
                </div>
                ...
                <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                    <div class="flex items-start">
                        <div class="flex items-center h-5">
                            <input id="guidelines" name="agreed_to_guidelines" type="checkbox" {{ old('agreed_to_guidelines', $post->agreed_to_guidelines) ? 'checked' : '' }} required
